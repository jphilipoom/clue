...
class OptionView(UIView):
    def __init__(self):
        ...
        nav_side.add_button("Host Game", style=UIFlatButton.STYLE_BLUE, size_hint=(1, 0.1))
        nav_side.add_button("Join Game", style=UIFlatButton.STYLE_BLUE, size_hint=(1, 0.1))
    ...
    def join_game(self):
        ...
        text_area = box.add(
            UITextArea(
                text=textwrap.dedent("""
                       Ask the game host for their id to join their game
                    """).strip(),
                font_name=DETAILS_FONT,
                font_size=16,
                text_color=arcade.uicolor.WHITE,
        ...
        self.user_input = box.add(UIInputText(width=150, font_name="Kenney Future"), column=1, row=1)
        
        self.join_button = box.add(
            UIFlatButton(text="Join", height=30, width=150, size_hint=(1, None)),
            column=0,
            row=3,
            column_span=2,
        )
    ...
    def on_join_game(self, event: UIOnClickEvent | None):
        print("joined the game with id : " + self.user_input.text.strip())
