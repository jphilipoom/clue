...
def draw_room(room_num):
    ...
    arcade.draw_lbwh_rectangle_filled(
        left_pos, bottom_pos, ROOM_SIZE, ROOM_SIZE, (214, 214, 214)
    )
    arcade.draw_text(
        ROOM_NAMES[room_num],
        left_pos + (ROOM_SIZE / 2),
        bottom_pos + (ROOM_SIZE / 2),
        (0, 0, 0),
        anchor_x="center",
        anchor_y="center",
    )
...
def draw_hallway(hallway_num):
    ...
    if hallway_num < 3:
        arcade.draw_lbwh_rectangle_filled(
            lb_pos1, bl_pos, HALLWAY_LENGTH, HALLWAY_WIDTH, (150, 150, 150)
        )
        arcade.draw_lbwh_rectangle_filled(
            lb_pos2, bl_pos, HALLWAY_LENGTH, HALLWAY_WIDTH, (150, 150, 150)
        )
    else:
        arcade.draw_lbwh_rectangle_filled(
            bl_pos, lb_pos1, HALLWAY_WIDTH, HALLWAY_LENGTH, (150, 150, 150)
        )
        arcade.draw_lbwh_rectangle_filled(
            bl_pos, lb_pos2, HALLWAY_WIDTH, HALLWAY_LENGTH, (150, 150, 150)
        )


def main():
    arcade.open_window(width=BOARD_SIZE, height=BOARD_SIZE, window_title="Clue-Less!")
    arcade.set_background_color((12, 3, 92))
    arcade.start_render()
