...
BOARD_LOCATIONS = ['Study', 'H1', 'Hall', 'H2', 'Lounge', 
             'H3','','H4','','H5',
...
def draw_board(players):
    ...
    for row in range(size):
        ...
        for col in range(size):
            str_row += "|" + BOARD_LOCATIONS[row*5 + col].center(15) + "|"
            locs.append(BOARD_LOCATIONS[row*5 + col])
            inserted_players[BOARD_LOCATIONS[row*5 + col]] = []
        str_row += "\n"
        ...
        for i in range(len(players)):
            for col in range(size):
                str_row += "|" + inserted_players[BOARD_LOCATIONS[row*5 + col]][i].center(15) + "|"
            str_row += "\n"
